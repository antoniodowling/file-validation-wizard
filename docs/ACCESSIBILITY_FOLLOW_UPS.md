...
| A11Y-004 | High | 1.3.1 Info and Relationships; 4.1.2 Name, Role, Value | The visual stepper changes current and completed states only through classes and color. Assistive technology receives the ordered list and labels, but not the current-step state. | Add `aria-current="step"` to the active step and expose completed/current wording without duplicating noisy announcements. | Accessibility-tree inspection at Steps 1, 2, and 3 plus screen-reader verification. |
| A11Y-005 | Medium | 1.3.1 Info and Relationships | The validation-results table headers do not declare `scope="col"`. Browsers generally infer these simple column relationships, so this is accessibility hardening rather than a confirmed failure. | Add `scope="col"` to all four column headers and retain a stable accessible name for the table or its surrounding section. | Accessibility-tree inspection and a screen-reader table-navigation check. |
